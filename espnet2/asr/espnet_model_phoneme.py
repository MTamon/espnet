...
class ESPnetASRModelWithPhoneme(AbsESPnetModel):
    ...
    def __init__(
        ...
        char_token_list: Union[Tuple[str, ...], List[str]],
        ...
        if sym_blank in char_token_list:
            self.char_blank_id = char_token_list.index(sym_blank)
        else:
            self.char_blank_id = 0
        if sym_blank in phone_token_list:
            self.phone_blank_id = phone_token_list.index(sym_blank)
        else:
            self.phone_blank_id = 0

        if sym_sos in char_token_list:
            self.char_sos = char_token_list.index(sym_sos)
        else:
            self.char_sos = char_vocab_size - 1
        if sym_sos in phone_token_list:
            self.phone_sos = phone_token_list.index(sym_sos)
        else:
            self.phone_sos = phone_vocab_size - 1

        if sym_eos in char_token_list:
            self.char_eos = char_token_list.index(sym_eos)
        else:
            self.char_eos = char_vocab_size - 1
        ...
        self.char_token_list = char_token_list.copy()
        ...
        if self.use_transducer_decoder:
            ...
            if not transducer_multi_blank_durations:
                from warprnnt_pytorch import RNNTLoss  # type: ignore

                self.criterion_transducer = RNNTLoss(
                    blank=self.char_blank_id,
                    fastemit_lambda=0.0,
                )
            else:
            ...
            if char_ctc_weight + phone_ctc_weight < 1.0:
                assert (
                    decoder is not None
            ...
            self.char_criterion_att = LabelSmoothingLoss(
        ...
        if char_ctc_weight == 0.0:
            self.char_ctc = None
        else:
            self.char_ctc = char_ctc
        if phone_ctc_weight == 0.0:
            self.phone_ctc = None
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        char_loss_ctc, char_cer_ctc = None, None
        ...
        if self.phone_ctc_weight != 0.0:
            ...
            stats["char_loss_ctc"] = (
                char_loss_ctc.detach() if char_loss_ctc is not None else None
            )
            stats["char_cer_ctc"] = char_cer_ctc
        ...
        if self.use_transducer_decoder:
            ...
            if (self.char_ctc_weight + self.phone_ctc_weight) != 1.0:
                loss_att, acc_att, cer_att, wer_att = self._calc_att_loss(
                    encoder_out, encoder_out_lens, text, text_lengths
                )

            # 3. CTC-Att loss definition
            if (self.char_ctc_weight + self.phone_ctc_weight) == 0.0:
                loss = loss_att
            elif (self.char_ctc_weight + self.phone_ctc_weight) == 1.0:
                loss = loss_ctc
            else:
                loss = (
                    loss_ctc
                    + (1 - (self.char_ctc_weight + self.phone_ctc_weight)) * loss_att
                )
    ...
    def encode(
        ...
        if self.encoder.interctc_use_conditioning or getattr(
            ...
            encoder_out, encoder_out_lens, _ = self.encoder(
                feats, feats_lengths, ctc=self.char_ctc
            )
        else:
    ...
    def nll(
        ...
        ys_in_pad, ys_out_pad = add_sos_eos(
            ys_pad, self.char_sos, self.char_eos, self.ignore_id
        )
    ...
    def _calc_att_loss(
        ...
        loss_att = self.char_criterion_att(decoder_out, ys_out_pad)
        acc_att = th_accuracy(
            decoder_out.view(-1, self.char_vocab_size),
            ys_out_pad,
    ...
    def _calc_batch_char_ctc_loss(
        ...
        # Calc CTC loss
        do_reduce = self.char_ctc.reduce
        self.char_ctc.reduce = False
        loss_ctc = self.char_ctc(encoder_out, encoder_out_lens, text, text_lengths)
        self.char_ctc.reduce = do_reduce
        return loss_ctc
